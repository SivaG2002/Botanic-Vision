import os
import allowed_extensions  # Import your allowed extensions file
...
def allowed_file(filename):
    return '.' in filename and \
           filename.rsplit('.', 1)[1].lower() in allowed_extensions.ALLOWED_EXTENSIONS
...
# Route for handling image prediction
    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            try:
                ...
                img = img.resize((model.input_shape[1:-1]))  # Resize based on model's expected shape (excluding batch dimension)
                img = np.array(img)
            ...
            flash('Allowed file types: ' + ', '.join(allowed_extensions.ALLOWED_EXTENSIONS))
